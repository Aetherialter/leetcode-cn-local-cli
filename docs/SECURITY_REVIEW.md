...
自动化覆盖文件目标、终端文本、授权端点、启动竞态、非法编码、worker 超时、提交总预算、安全重试和退出码。真实浏览器、Cookie 和提交只能进行维护者明确授权且脱敏的手动验收；`v0.9.0` 已在 Windows 对 Chrome 与 Edge 各完成一次真实授权登录，2026-08-09 又在明确授权下完成 10 次真实提交和对应的 `lc check` 重查。验收没有输出 Cookie，但不消除明文 Session 和用户代码执行风险。
