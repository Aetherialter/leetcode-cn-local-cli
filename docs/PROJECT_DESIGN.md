...
适用范围：从当前 `v0.7.2` 演进到首个稳定版本 `v1.0`。本文只详细设计当前单一项目；数据库、全栈、AI 和双产品线属于 `v1.0` 之后的方向。
...
| 工作区模型 | v0.7.x 已确认使用单个 `solution.py`；v0.8 以后模型按 PB-008 决策，不得提前冻结公开 API |
...
现有 `.leetcode_local_cli/session.json` 与 `.aether_lc/session.json` 只作为迁移输入：
...
| 决策 | 最晚时间 |
| --- | --- |
| PB-008：正式工作区根目录、文件布局与覆盖规则 | v0.8 实施前形成阶段结论，v0.14 API 冻结前形成长期结论 |
| PB-009：Linux 无秘密服务时的交互与降级策略 | v0.8 完成前 |
| PB-001、PB-002：本地测试入口、输出和调试语义 | 下一次修改 `lc test` 前 |
