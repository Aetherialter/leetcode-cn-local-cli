...
适用范围：从当前 `v0.9.0` 演进到首个稳定版本 `v1.0`。本文只详细设计当前单一项目；数据库、全栈、AI 和双产品线属于 `v1.0` 之后的方向。
...
这套结构主要供 CLI 渲染，是否作为 v1.0 顶层公开 API，需要在 `v0.13` 决策。
...
- 内联协议的具体支持矩阵在 `v0.13` 通过原型确定，不提前绑定第三方库。
...
### v0.9：浏览器自动登录与本地执行可靠性

目标：完成日常 Chrome/Edge 显式授权自动登录闭环，并修复本地测试、编码和提交退出码中影响真实使用与自动化判断的可靠性缺口。

交付物：

- Chrome → Edge → 手动 Cookie 的默认登录顺序，以及单一浏览器显式选择。
- 日常浏览器 `DevToolsActivePort` 授权发现、自动拉起、遗留端点和 403 恢复。
- 交互式 `lc test`、JSON Lines stdin、逐组超时和 worker 重启。
- 严格 UTF-8/UTF-8 BOM 解码边界与显式未实现模板。
- 远程提交的 Accepted/非 Accepted 机器退出码。

验收：

- Chrome 与 Edge 均完成用户明确允许的真实登录验收。
- 自动化覆盖浏览器未运行、授权缺失、遗留端点、身份错误和启动竞态。
- 本地执行和提交结果的终端文案与退出码一致。
- Ruff、Pyright、完整测试、wheel/sdist 和入口检查通过。

### v0.10：领域模型、异常与 Python API 预览
...
| 决策 | 最晚时间 |
| --- | --- |
| 单文件默认工作区是否成为长期模型 | v0.15 API 冻结前形成长期结论 |
| 系统秘密存储与明文 Session 迁移策略 | 实施秘密存储的版本开始前 |
| PB-C12：本地测试入口、基础输出和超时已确认；PB-C13：UTF-8 编码边界已确认；PB-002：verbose/Doctor 扩展输出仍待定 | 扩展本地调试输出前 |
| PB-C14：提交退出码已确认；PB-004：提交前本地预检仍待定 | 下一次修改提交预检前 |
| PB-007：编辑器配置模型 | 编辑器设置命令实施前 |
| UI 使用普通 CLI、TUI 或组合 | v0.14 实施前 |
| 具体终端图片协议与依赖 | v0.13 实施前 |
| PB-012：v1.0 正式支持哪些 Python 小版本 | v0.16 测试矩阵冻结前 |
| 题面文档块是否作为公开 API | v0.15 API 冻结前 |
