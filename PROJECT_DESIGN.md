...
状态：`Draft 0.1`，等待审查，不代表已批准实施。

适用范围：从当前 `v0.7.1` 演进到首个稳定版本 `v1.0`。本文只详细设计当前单一项目；数据库、全栈、AI 和双产品线属于 `v1.0` 之后的方向。
...
| 工作区模型 | 暂未决定；不得提前把公开 API 固定为全局唯一 `solution.py` |
...
| 决策 | 最晚时间 |
| --- | --- |
| 单文件、自动归档或多题工作区 | v0.14 API 冻结前 |
| UI 使用普通 CLI、TUI 或组合 | v0.13 实施前 |
| 具体终端图片协议与依赖 | v0.12 实施前 |
| Linux 无秘密服务时的交互策略 | v0.8 完成前 |
| v1.0 正式支持哪些 Python 小版本 | v0.15 测试矩阵冻结前 |
