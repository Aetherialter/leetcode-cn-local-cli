...
- `v0.7.0`: 新增 uv 全局安装、跨平台引导脚本和 PyPI Trusted Publisher 发布流程。
- `v0.7.1`: 修正安装版命令建议，并让 GitHub Release 使用版本化标题和说明。
- `v0.7.2`: 收紧终端渲染、Cookie、远端响应和安装流程的安全边界，并让 `lc doctor` 默认不执行解题文件。
...
- 标签发布工作流在 Linux、macOS 和 Windows 上通过后才允许上传 PyPI。

### v0.7.1: 安装版命令提示修复

目标：让通过 PyPI 安装的用户直接使用 `lc`，并确保 GitHub Release 在发布时带有可读的版本说明。

更新内容：

- 将 `lc doctor` 的 `uv run lc ...` 建议修正为 `lc ...`。
- 同步修正 service 层的登录失败提示。
- 增加安装版命令提示的回归测试。
- 为每个版本维护标题和 Release Notes，并由发布工作流自动使用。

### v0.8: 工作区初始化

目标：通过 `lc init` 初始化独立刷题目录，并正式分离工具安装目录、用户配置和本地工作区。

工作区根目录、子目录调用、`solution.py` 位置和覆盖规则仍属于 [PB-008](PRODUCT_BOUNDARIES.md#pb-008v08-工作区模型)，必须在实施前完成定界。v0.7.x 的单题、单文件和普通文件强制覆盖行为不因此改变。

计划范围：

- 新增 `lc init [目录]`。
- 使用工作区配置文件识别根目录。
- 根据 PB-008 的最终结论实现初始化与已有 `solution.py` 的处理，不预先固定覆盖语义。
...
- PyPI 发布已在 v0.7.0 接入后，持续完善自动发布与安装包验证。
- 暂不建设大型文档站。
...
当前行为边界统一记录在 [PRODUCT_BOUNDARIES.md](PRODUCT_BOUNDARIES.md)，实现前按其中的决策规则同步 README、路线图、长期设计、安全清单、代码和测试。
